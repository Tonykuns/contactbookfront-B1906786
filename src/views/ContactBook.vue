...
<div class="list row">
    ...
    <div class="col-md-6">
        ...
        <ul class="list-group">
            <li class="list-group-item" :class="{ active: index == activeIndex }" v-for="(contact, index) in contacts" :key="contact.id" @click="setActiveContact(contact, index)">
                {{ contact.name }}
            </li>
        </ul>
    ...
    <div class="col-md-6">
        <div v-if="currentContact">
            ...
            <router-link :to="'/contacts/' + currentContact.id"><span class="badge badge-warning">Hiệu chỉnh</span></router-link>
        </div>
    </div>
</div>
</template>
